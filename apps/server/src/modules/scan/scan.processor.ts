...
const SAVE_SCHEMA_TOOL: FunctionDeclaration = {
  ...
  description:
    "Save the generated OpenAPI 3.0 schema based on the frontend code analysis.",
  parameters: {
    ...
    properties: {
      paths: {
        ...
        properties: {}, // Dynamic structure - properties defined at runtime
      },
    },
    required: ["paths"],
  },
...
export class ScanProcessor extends WorkerHost {
  ...
  async process(
    job: Job<{ projectId: string; codeSnippets: string }>,
  ): Promise<any> {
    ...
    try {
      ...
      if (toolCall.name === "save_openapi_schema") {
        ...
        return {
          success: true,
          endpointsGenerated: Object.keys(schemaArgs.paths).length,
        };
      }
  ...
  private async seedDatabase(projectId: string, paths: any) {
    ...
    for (const [path, methods] of Object.entries(paths)) {
      for (const [method, details] of Object.entries(methods as any)) {
        const responseSchema = (details as any).responses?.["200"]?.content?.[
          "application/json"
        ]?.schema;
        ...
        if (responseSchema) {
          try {
            ...
            fakeData = await JSONSchemaFaker.resolve(responseSchema);
          } catch (e) {
    ...
    if (endpointsToSave.length > 0) {
      ...
      this.logger.log(
        `Seeded ${endpointsToSave.length} endpoints for project ${projectId}`,
      );
    }
  }
}
