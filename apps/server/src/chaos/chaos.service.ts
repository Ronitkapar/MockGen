...
export class ChaosService {
  ...
  async getChaosConfig(projectId: string): Promise<ChaosSettings> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to parse chaos config for project ${projectId}`,
      );
      return { latencyMs: 0, forceError: false };
    }
  }

  async setChaosConfig(
    projectId: string,
    settings: UpdateChaosInput,
  ): Promise<ChaosSettings> {
    const key = this.getKey(projectId);
    await this.redis.set(key, JSON.stringify(settings), "EX", this.TTL_SECONDS);

    this.logger.log(
      `Chaos config updated for ${projectId}: ${settings.latencyMs}ms, Error: ${settings.forceError}`,
    );
    return settings;
  }
}
